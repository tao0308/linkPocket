...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    public ShortLinkCreateRespDTO createShortLink(ShortLinkCreateReqDTO requestParam) {
        ...
        try{
            ...
            shortLinkGotoMapper.insert(linkGotoDO);
                ...
                throw new ServiceException(String.format("短链接生成: %s 生成重复",fullShortUrl));
        }
    ...
    private ShortLinkStatsRecordDTO buildLinkStatsRecordAndSetUser(String fullShortUrl, ServletRequest request, ServletResponse response){
        ...
        Runnable addResponseCookieTask = () -> {
            ...
            stringRedisTemplate.opsForSet().add(SHORT_LINK_START_UV_KEY + fullShortUrl, uv.get());
        };
        if (ArrayUtil.isNotEmpty(cookies)) {
            Arrays.stream(cookies)
                    ...
                    .ifPresentOrElse(each -> {
                        uv.set(each);
                        Long uvAdded = stringRedisTemplate.opsForSet().add(SHORT_LINK_START_UV_KEY + fullShortUrl, each);
        ...
        Long uipAdded = stringRedisTemplate.opsForSet().add(SHORT_LINK_START_UIP_KEY + fullShortUrl, remoteAddr);
